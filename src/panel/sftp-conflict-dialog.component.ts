...
import type { ConflictFileInfo } from './panel-types'
import { formatDate, formatSize } from './panel-format'
      ...
      <div class="dialog conflict-dialog">
        ...
        <div class="conflict-desc">
          ...
          <ng-template #fileConflictDesc>
            {{ data.isSamePane ? '目标位置已存在同名文件' : (data.direction === 'download' ? i18n.t('conflict.existsLocal') : i18n.t('conflict.existsRemote')) }}
          </ng-template>
        </div>
        <div class="conflict-compare">
          <div class="conflict-side conflict-side-remote">
            <div class="conflict-side-title">{{ data.isSamePane ? '📄 源文件' : '☁️ ' + i18n.t('conflict.remoteFile') }}</div>
          ...
          <div class="conflict-side conflict-side-local">
            <div class="conflict-side-title">{{ data.isSamePane ? '📂 目标文件（已存在）' : '📁 ' + i18n.t('conflict.localFile') }}</div>
            <div class="conflict-file-info">
              <div class="conflict-info-row" [class.conflict-diff]="data.localSize !== data.remoteSize">
                <span class="conflict-label">{{ i18n.t('conflict.size') }}</span>
