...
@Component({
  ...
  template: `
    <div class="context-menu" *ngIf="menuVisible"
      ...
      <!-- 4. 重命名 / 删除 -->
      <div class="ctx-item" (click)="menuAction.emit('rename')" *ngIf="singleSelected">{{ i18n.t('app.rename') }}</div>
      <div class="ctx-item ctx-danger" *ngIf="hasSelection" (click)="menuAction.emit('delete')">{{ i18n.t('app.delete') }}</div>
      ...
      <!-- 7. 面板操作 -->
      <div class="ctx-item" (click)="menuAction.emit('refresh')">{{ i18n.t('app.refresh') }}</div>
    ...
    <div class="context-menu" *ngIf="headerVisible"
...
export class SftpContextMenuComponent {
