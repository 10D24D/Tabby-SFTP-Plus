...
| **👁️ 查看 / 编辑** | 内置文本/图片查看器与文本编辑器（本地+远程）；可额外指定可编辑扩展名，或允许编辑所有类型（二进制仍保护）；图片支持同目录上一张/下一张导航；支持「复制」「复制选中」；文本查看器/编辑器支持右键复制、剪切、粘贴、全选；支持「以文本方式查看」和「在系统中打开/编辑」 |
| **🎨 文件图标** | 内置彩色 SVG 文件/文件夹图标；符号链接 / 快捷方式叠加角标；支持按扩展名自定义映射、自定义图标目录、禁用内置图标和替换文件夹图标 |
| **🔄 拖拽传输** | 跨栏拖拽即上传/下载，支持文件夹递归传输；<br />支持从系统资源管理器/桌面拖入到本地或远程面板 |
| **⬆️ 右键传输** | 本地面板右键上传、远程面板右键下载（多选批量） |
| **⚡ 高速传输** | 目录内多文件并行传输（并发数 1-10 可调）；海量小文件走 tar 打包通道（本地打包→单文件传输→对端解包），大幅减少网络往返，可在设置页关闭；批量删除优先 SSH `rm -rf` / `fs.rm(recursive)` |
| **🔖 书签系统** | 全局书签（所有连接可见）+ 连接书签（仅当前 SSH 可见）；可按范围分组或扁平混排，分组顺序可拖拽 |
| **📋 传输日志** | 记录所有操作历史，含「编辑加载/编辑保存」分类，支持筛选、统计、JSON 导出 |
| **⚡ 传输控制** | 进度条显示（含百分比）、暂停/继续/取消、断点续传、实时速度；tar 打包通道失败自动回退常规逐文件传输；传输记录标注目录传输方式（⚡ 快速模式 / 📦 tar 打包 / 文件数） |
| **⚠️ 冲突处理** | 文件冲突时弹出对比界面（显示上传⬆/下载⬇方向），支持覆盖/跳过/重命名，可批量操作；覆盖合并有独立进度面板；可计算两端内容摘要并在对话框展示，内容未变可自动跳过，内容不同时标 ≠ |
...
| **🔍 过滤排序** | 关键词过滤、多列排序（点击列头）、可配置显示列；焦点在文件列表时键入文件名前缀即可跳转选中（资源管理器式，支持中文输入法） |
| **🧭 路径模式** | 三选一：`off` / `remember`（路径记忆）/ `sync`（与终端同步）；可设默认值 |
| **🎨 主题系统** | 7 种预设 + 自定义配色，支持跟随 Tabby 系统主题 |
| **⌨️ 面板快捷键** | 可自定义面板开关快捷键，支持录制/清除/冲突提示；面板操作（删除/重命名/刷新/返回上级/后退/前进）与右键菜单常用动作均支持**多键绑定**与鼠标侧键，并提供已占用快捷键清单 |
| **🌐 国际化** | 24 种语言（简中 / 英完整，其余缺失键回退英文），自动检测 Tabby/浏览器语言 |
...
| **书签弹窗** | 可按「连接书签」和「全局书签」分组，或扁平混排；支持拖拽排序和快速跳转 |
...
| **tar 打包加速** | 文件夹传输走 tar 打包通道（默认开启）；打包或解包失败时自动回退常规逐文件传输 |
| **默认上传/下载路径** | 分别设置上传目标目录和下载目标目录；留空时使用当前面板目录 |
| **默认路径模式** | 新连接首次打开面板时的路径模式（off / remember / sync） |
| **默认显示隐藏文件** | 新连接首次打开面板时是否显示隐藏文件 |
| **额外可编辑扩展名** | 在内置文本白名单之外，额外允许内置编辑器打开的扩展名（如 `service, tf, plist`）；支持 `.conf` / `*.conf` / `conf` 写法 |
| **允许编辑所有文件类型** | 忽略扩展名白名单（默认关）；二进制内容仍会拦截，避免用文本编辑器损坏文件 |
| **文件图标** | 选择图标资源目录、配置扩展名映射、禁用内置图标和替换文件夹图标 |
| **工具栏自定义** | 拖拽排序工具栏按钮、隐藏不常用项 |
| **面板快捷键** | 自定义面板开关快捷键；面板操作（删除/重命名/刷新/返回上级/后退/前进）与右键菜单常用动作可绑定多个按键或鼠标侧键（Mouse3/Mouse4），含已占用快捷键清单 |
...
当前开发版本 **v2.3.1**（2026-09-18）— [完整更新日志](CHANGELOG.md)
...
│       └── core/                        # 逻辑/工具/类型：传输、冲突、摘要、拖放、剪贴板、路径等
├── locale/                              # 24 语言 GNU gettext .po
├── tabby-plugin-common/                 # 跨插件公共工具（theme / utils）
├── dist/                                # 构建输出（index.js + package.json，不入库）
├── package.json
└── webpack.config.js
```
...
- [@Purgepyro](https://github.com/Purgepyro) — PR #18 新增自定义可编辑扩展名、允许编辑全部类型，并修复手动安装后内置图标空白
- [@waylandun](https://github.com/waylandun) — Issue #21 反馈编辑器粘贴会进终端，PR #22 在面板根隔离文本按键（macOS 实测）
