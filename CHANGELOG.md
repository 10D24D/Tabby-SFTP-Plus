...
- **面板快捷键多绑定** — 一个动作现在可绑定多个按键，设置页以标签（chip）形式展示，`+` 追加、`×` 移除；旧的单键配置自动迁移，无需重新设置。
- **鼠标侧键可配置** — 原先硬编码的鼠标后退（Mouse3）/ 前进（Mouse4）纳入快捷键配置，可改绑到任意动作，也可清除；新增 `forward`（前进）动作，与 `back`（后退）对称。
- **右键菜单动作可绑快捷键** — 上传 / 下载 / 新建文件夹 / 新建文件 / 属性 / 复制路径 六个常用右键动作支持绑定快捷键（默认留空，不影响既有操作）；仅在对应面板存在选中项时响应，不会抢占其它功能的键位。
- **已占用快捷键清单** — 设置页新增只读折叠区，按面板 / 查看器 / 对话框分组列出固定占用的键位，便于排查冲突。
- **tar 打包加速开关** — 新增 `transferTarAcceleration` 设置（默认开启），可按需关闭文件夹传输的 tar 打包通道。
- **传输记录模式标签** — 传输记录中显示目录传输方式：⚡ 快速模式、📦 tar 打包、标准模式显示本次传输的文件数量。

### 🐛 修复

- **拖拽目录覆盖产生重复记录** — 拖入同名目录并选择覆盖后，传输记录会出现「一条失败 + 一条成功」两条同目录条目。原因是面板侧的合并上传适配器漏转发了复用记录 ID，导致冲突入队时已标记为失败的那条记录无法被翻正。修复后拖拽覆盖只保留一条成功记录。
- **传输记录上传方向显示反转** — 上传条目误显示为「远端 → 本地」方向图标（下载方向恰好正确，故仅上传受影响）。

### 🎨 改进

- **tar 通道失败平滑回退** — tar 打包通道在打包、传输或解包任一环节失败时，不再直接中止整个传输，而是自动回退到常规逐文件传输，并清理服务端残留的临时解包目录与不完整目标。

### 🌐 多语言

- 补充本版本新增设置、快捷键与传输记录相关文案（24 种语言）。

### 🔧 技术 / 构建

- 新增 i18n 维护脚本 `scripts/add-multi-hotkey-i18n.mjs`、`scripts/add-transfer-mode-i18n.mjs`（幂等，可重复执行）。
- 版本号 bump 至 **2.2.0**。
