...
- **文件图标系统** — 内置彩色 SVG 文件/文件夹图标，支持按扩展名映射、自定义 SVG 资源目录、禁用内置图标和替换文件夹图标；文件列表与属性对话框统一使用同一套图标。
- **默认上传/下载路径** — 设置页可分别配置上传目标目录和下载目标目录，未配置时继续使用当前面板目录。
- **文本右键菜单** — 查看器和编辑器新增复制、复制选中、剪切、粘贴、全选等文本操作；大文件选区检测避免无必要的全文复制造成卡顿。

### 🐛 修复

- **远程符号链接目录无法进入** — 通过跟随 `stat` 并在必要时解析 `readlink`，正确识别指向目录的远程符号链接。
- **Windows 快捷方式处理** — 识别 `.lnk` 目标文件/目录，打开、进入和拖拽时使用真实目标路径。
- **路径安全与规范化** — 本地新建/重命名和远程新建操作增加越界路径防护；书签路径统一规范化，避免多窗口删除后的书签复活。
- **传输取消与会话恢复** — 并发目录传输取消时中止全部在途子流；会话替换前终止旧传输并提示用户重试，避免幽灵写入。
- **快捷键与焦点** — 模态对话框、终端焦点、面板焦点和删除确认的事件处理更加明确，避免快捷键误触或重复执行。

### 🎨 改进

- **浮动面板适配** — 面板尺寸以百分比保存并跟随窗口自适应；拖拽增加移动阈值，跨插件共享置顶层级，减少误移动和面板遮挡。
- **配置一致性** — 改进配置缓存失效和面板快捷键清除状态的持久化，避免多窗口或重启后恢复旧配置。
- **多语言** — 补充本版本新增设置、图标和文本菜单相关文案。

### 🔧 技术 / 构建

- 新增内置图标资源复制脚本 `scripts/copy-icons.mjs`，生产构建自动将 SVG 图标复制到 `dist/assets/icons/`。
- 版本号 bump 至 **2.1.0**。
